...
namespace Dww\Shiprestriction\Observer;

use Magento\Config\Model\ResourceModel\Config;

class DisableOutput implements \Magento\Framework\Event\ObserverInterface
{
    public const DWW_CONFIG = 'shiprestriction/shiprestriction/enable';
    /**
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        if ($this->request->getParam(\Magento\Store\Model\ScopeInterface::SCOPE_STORE)) {
            ...
            $scopeCode = $this->storeManager->getStore(
                $this->request->getParam(\Magento\Store\Model\ScopeInterface::SCOPE_STORE))->getCode();
        ...
        $moduleConfig = $this->_scopeConfig->getValue(self::DWW_CONFIG, $scopeType);
        if ((int)$moduleConfig == 0) {
            $disable = true;
        }
        $moduleName = 'Dww_Shiprestriction';
