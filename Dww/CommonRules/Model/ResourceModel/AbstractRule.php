...
namespace Dww\CommonRules\Model\ResourceModel;
...
abstract class AbstractRule extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function __construct(
        ...
        \Dww\Shiprestriction\Model\Serializer $serializer,
        $connectionName = null
    ) {
    ...
    protected function recursiveFindAttributes($loop)
    {
        ...
        foreach ($arrayIterator as $key => $value) {
            if ($key == 'type' && $value == \Dww\CommonRules\Model\Rule::SALES_RULE_PRODUCT_CONDITION_NAMESPACE) {
                $nextAttribute = true;
            }
     ...
     *
     * @param \Dww\CommonRules\Model\Rule $model
     *
     * @return array
